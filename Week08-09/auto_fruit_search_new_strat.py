...
if __name__ == "__main__":
    ...
    while start:
        ...
        operate.update_slam(drive_meas)
        operate.robot_pose = operate.ekf.robot.state[:3,0]
        operate.notification = f"Robot Pose : [{np.round(operate.robot_pose[0], decimal=2)}, {np.round(operate.robot_pose[1], decimal=2)}, {np.round(operate.robot_pose[2], decimal=2)}]"
        # print(operate.robot_pose)
        operate.record_data()
        operate.save_image()
        operate.detect_target()
